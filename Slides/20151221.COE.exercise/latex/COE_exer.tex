...
\frame{
  \frametitle{字母次数}
     \only<1-2>{\includegraphics[width=0.9\hsize]{pic/字母次数-o.png}
      ...
      \end{itemize}
      \only<2>{\includegraphics[width=0.99\hsize]{pic/字母次数-bad2.png}\\ }
     }
     \only<3>{\includegraphics[width=0.7\hsize]{pic/字母次数.png}\\ }
}
...
\frame{
  \frametitle{1.删除数组中的元素（链表）}
     ...
     \only<8-9>{\includegraphics[width=0.6\hsize]{pic/删除5.png}}
     \end{columns}
}
...
\frame{
  \frametitle{1.旋转输出矩阵}
     ...
     \begin{columns}
      \column{0.3\hsize}	
     \begin{itemize}
      \item 和上机模拟的`二维数组回形遍历`一样，输出的时候两端夹逼
     \end{itemize}
...
\frame{
  \frametitle{5.计算反序数}
     ...
     \column{0.7\hsize}
     \only<2>{\includegraphics[width=0.85\hsize]{pic/计算反序数-good.png}}
     \end{columns}
     }
}
\frame{
...
\frame{
  \frametitle{7.奇数单增序列}
     ...
     \column{0.7\hsize}
     \only<2>{\includegraphics[width=0.85\hsize]{pic/奇数单增序列-good.png}}
     \end{columns}
     }
}	
\frame{
...
\frame{
  \frametitle{9.前缀表达式}
     ...
     \only<1>{\centering\includegraphics[width=0.65\hsize]{pic/前缀表达式.png}}
     \only<2->{
     \begin{columns}
      \column{0.3\hsize}	
     \begin{itemize}
	...
	\end{itemize}
     ...
     \only<3>{\includegraphics[width=0.85\hsize]{pic/前缀表达式-good2.png}}
     \end{columns}
     }
}
...
\frame{
  ...
  int num[300] = \{0\};
  头文件
  本地运行过不了样例的，提交上去99.999999999\%过不了
  很奇怪的数（-1208899956），数组越界！
  机器很死板，你得按照他的格式来
}
